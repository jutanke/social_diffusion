...
class EMA(nn.Module):
    ...
    good values for models you plan to train for a million or more steps
    (reaches decay factor 0.999 at 31.6K steps, 0.9999 at 1M steps), gamma=1,
    power=3/4 for models you plan to train for less (reaches decay factor
    0.999 at 10K steps, 0.9999 at 215.4k steps).
    Args:
    ...
    def __init__(
        self,
        model,
        ema_model=None,  # noqa E501 if your model has lazylinears or other types of non-deepcopyable modules, you can pass in your own ema model
        beta=0.9999,
        ...
        if not exists(self.ema_model):
            try:
                ...
                print(
                    "Your model was not copyable. Please make sure you are not using any LazyLinear"  # noqa E501
                )
                exit()
    ...
    def update_moving_average(self, ma_model, current_model):
        ...
        for (name, current_buffer), (_, ma_buffer) in zip(
            ...
            self.get_buffers_iter(ma_model),  # noqa E501
        ):
            if name in self.ignore_names:
                continue
